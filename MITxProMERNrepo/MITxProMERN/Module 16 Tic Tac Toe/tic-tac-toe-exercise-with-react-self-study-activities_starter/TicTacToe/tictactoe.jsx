...
const Board = () => {
  ...
  const Scoreboard = () => {
    ...
    React.useEffect(() => {
      // Attach the event listener for updating the scoreboard
      const scoreboardUpdater = event => {
        updateScoreboard(event.detail.winner);
      };
      document.addEventListener('updateScoreboard', scoreboardUpdater);
  
      return () => {
        // Clean up the event listener when the component unmounts
        document.removeEventListener('updateScoreboard', scoreboardUpdater);
      };
    }, []);
    ...
    return (
      <div className="scoreboard">
        <p>Player X Score: {playerXWins}</p>
        <p>Player O Score: {playerOWins}</p>
        <p>Draws: {draws}</p>
