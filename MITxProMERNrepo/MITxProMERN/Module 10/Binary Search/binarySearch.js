...
const searchForAlphabeticalIndex = ([stringToFindIndexForFirstLetter], alphabetArray = generateAlphabetArray()) => {
    ...
    return true;
        ...
        if (array[midIndex] > letter) {
    // This return statement will continue the recursion by narrowing the search to the lower (start to middle) portion of the dataset
    return recursiveBinarySearch(array, letter, startIndex, midIndex - 1);
  } else {
    // This return statement will continue the recursion by narrowing the search to the upper (middle to end) portion of the dataset
